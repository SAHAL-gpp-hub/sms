...
class TestMarksGrid:
    ...
    def test_mark_student_absent(self, api, create_student):
        ...
        if r2.status_code == 200:
            ...
            if student_result:
                absent_subjects = [
                    sub for sub in student_result.get("subjects", [])
                    if sub.get("grade") == "AB"
                ]
...
class TestGradeCalculation:
    ...
    def _setup_student_with_marks(self, api, create_student, score, class_id=1):
        ...
        entries = [make_mark(sid, exam_id, sub["id"], marks=score) for sub in subjects]
    ...
    def test_same_percentage_same_rank(self, api, create_student):
        ...
        if r.status_code == 200:
            ...
            if r1 and r2:
                assert abs(r1 - r2) <= 1, \
                    f"Same score ranks should be same/consecutive, got {r1} and {r2}"
