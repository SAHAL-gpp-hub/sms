...
function GradePill({ grade }) {
  ...
  return (
    <span style={{
      ...
      borderRadius: '6px', fontSize: '11.5px', fontWeight: 800,
...
function ExamCard({ exam }) {
  ...
  touchAction: 'manipulation',
      ...
      >
        <div style={{ textAlign: 'left' }}>
          ...
          <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px', fontWeight: 600 }}>
            {Math.round(exam.total_marks || 0)}/{Math.round(exam.max_marks || 0)} · {(exam.percentage || 0).toFixed(1)}%
          </div>
        </div>
      ...
      {/* Expanded subjects */}
      {expanded && (
        <div style={{ borderTop: '1px solid #f0f7f7', padding: '4px 0 8px' }}>
          {/* Summary strip */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '6px', padding: '8px 14px 10px' }}>
            {[
          ...
          {(exam.subjects || []).map((sub, i) => {
            ...
            return (
              <div key={i} style={{
                display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                padding: '9px 16px', borderBottom: i < (exam.subjects.length - 1) ? '1px solid #f0f7f7' : 'none',
              }}>
          ...
          {exam.exam_id && (
            <div style={{ padding: '8px 14px 4px' }}>
              <a
                href={`/api/v1/portal/me/marksheet/${exam.exam_id}`}
                target="_blank" rel="noreferrer"
                style={{
                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px',
                  padding: '10px', borderRadius: '10px',
                  background: '#0d737710', color: '#0d7377',
...
export default function PortalResults() {
  ...
  if (loading) return (
    <div>
      {[1,2,3].map(i => (
        <div key={i} style={{ height: '80px', borderRadius: '16px', background: 'linear-gradient(90deg, #f0f7f7 25%, #e0eded 50%, #f0f7f7 75%)', backgroundSize: '200% auto', animation: 'portalShimmer 1.5s linear infinite', marginBottom: '10px' }} />
      ))}
    </div>
  )
  ...
  if (error) return (
    <div style={{ textAlign: 'center', padding: '40px 20px', color: '#64748b' }}>
      ...
      <div style={{ fontWeight: 700, fontSize: '15px' }}>Couldn't load results</div>
      <div style={{ fontSize: '13px', marginTop: '6px' }}>Please try again later</div>
    </div>
  )
  ...
  return (
    <>
      <div style={{ marginBottom: '14px' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 900, color: '#0f172a', letterSpacing: '-0.02em' }}>My Results</h2>
        <p style={{ fontSize: '12.5px', color: '#64748b', marginTop: '2px', fontWeight: 600 }}>
          {results.length} exam{results.length !== 1 ? 's' : ''} · tap to expand
        </p>
      </div>
      ...
      {results.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px 20px', background: 'white', borderRadius: '16px' }}>
          ...
          <div style={{ fontSize: '13px', color: '#64748b', marginTop: '4px' }}>Results will appear once your teacher enters marks</div>
        </div>
      ) : (
        results.map((exam, i) => <ExamCard key={i} exam={exam} />)
      )}
    </>
  )
}
