...
export function ReceiptModal({ payment, onClose }) {
  ...
  /**
   * Open the standardised PDF receipt(s) generated by the backend
   * (fee_receipt_template.html via WeasyPrint).  This is the single
   * canonical receipt format used everywhere in the system: immediate
   * admin print, later download, and WhatsApp notifications all use
   * the exact same PDF layout.
   *
   * If the payment spanned multiple allocations each one is a separate
   * FeePayment row and therefore a separate receipt PDF.
   */
  const handlePrint = async () => {
    const ids = payment.payment_ids?.length
      ? payment.payment_ids
      : [payment.id].filter(Boolean)

    for (const pid of ids) {
      try {
        await feeAPI.downloadReceipt(pid)
      } catch {
        // individual receipt failure — continue opening the rest
      }
    }
  }
