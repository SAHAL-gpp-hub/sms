...
export function ReceiptModal({ payment, onClose }) {
  ...
  const handlePrint = () => {
    ...
    w.document.write(`
      ...
      <table class="alloc-table">
        ...
        <tbody>
          \${payment.allocations.map(a => \`
            <tr>
              <td>\${a.fee_head_name}</td>
              <td>₹\${Number(a.amount_applied).toLocaleString('en-IN')}</td>
              <td>₹\${Number(a.balance_after).toLocaleString('en-IN')}</td>
            </tr>
          \`).join('')}
        </tbody>
      </table>
      \` : \`
      <table>
        <tr><td>Fee Head</td><td>\${payment.fee_head_name || '—'}</td></tr>
        <tr><td>Instalment</td><td>\${payment.instalment_seq || 'Full payment'}</td></tr>
      </table>
      \`}
      
      <hr/>
      <table>
        <tr><td>Total Paid</td><td class="amount">₹\${Number(payment.total_amount || payment.amount_paid).toLocaleString('en-IN')}</td></tr>
        <tr><td>Total Balance After</td><td style="color:\${(payment.total_balance_after || payment.balance_after) > 0 ? '#dc2626' : '#16a34a'}">
          ₹\${Number(payment.total_balance_after ?? payment.balance_after ?? 0).toLocaleString('en-IN')}
        </td></tr>
      </table>
